...
async function get(
  request: NextRequest,
): Promise<NextResponse<FlightsResponse | { error: string }>> {
  ...
  try {
    ...
    const originAirports: Airport[] = await prisma.airport.findMany({
      where: {
        OR: [
    ...
    const destinationAirports: Airport[] = await prisma.airport.findMany({
      where: {
        OR: [
          {
            city: {
    ...
    const originAirportIds = originAirports.map((airport: Airport) => airport.id);
    ...
    const indirectFlights: Array<{ firstLeg: Flight; secondLeg: Flight }> = [];
    ...
    return NextResponse.json({
      results: [
        ...directFlights.map((flight: Flight) => ({ legs: 1, flights: [flight] })),
        ...indirectFlights.map((flight: { firstLeg: Flight; secondLeg: Flight }) => ({
          legs: 2,
