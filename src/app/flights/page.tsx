...
export default function FlightSearch() {
  ...
  const onSubmit = async (data: FlightSearchForm) => {
    try {
      ...
      // Retrieve the token from storage or set a fallback for development
      let token = localStorage.getItem('token')
      if (!token) {
        // Set a fallback token for testing purposes (replace with a valid token)
        token = 'test-token'
      ...
      // Extract flights from the expected response structure
      if (response.results && response.results.length > 0) {
        setFlights(response.results[0].flights || [])
      } else {
        setFlights([])
      }
      setIsLoading(false)
